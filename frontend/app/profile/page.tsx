...
import { AVATAR_PRESETS, presetKeyToUrl, resolveAvatarSrc, AvatarValue } from "@/components/avatarPresets";
...
export default function ProfilePage() {
  ...
  const fetchProfile = useCallback(async () => {
    ...
    setError("");
    try {
      const res = await fetch(`${API}/api/profile/me`, { credentials: "include", headers: authHeaders() });
  ...
  const handleResumeDelete = async () => {
    try {
      const res = await fetch(`${API}/api/profile/me/resume`, { method: "DELETE", credentials: "include", headers: authHeaders() });
      const data = await res.json();
